...
Two accounts: **dev** (`449952321849`, internal testing) and **prod** (`550271577754`, public consumer registrations). GitHub Actions uses Environments `dev` / `prod` with the same secret name `AWS_ROLE_ARN`. Deploy shared account stacks locally (`cdk:deploy:shared`) — they are not part of `cdk deploy --all`. See `docs/SETUP.md`.
