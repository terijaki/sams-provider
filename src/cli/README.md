...
5. **EventBridge** rule `sams-provider-<consumer-id>` on bus `sams-provider`, targeting that SQS ARN, matching `source: sams-provider` and every current `detail-type` in the `sams-provider-events` package (`EventType` / `ProjectionEventType`).
