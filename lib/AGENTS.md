...
- **`shared`:** account singletons `GitHubOidcStack` and `BudgetStack` (`Foo-Dev` / `Foo-Prod`, no branch). Termination-protected. Not part of default `cdk deploy --all`. Prod CI deploys them after merge to `main`; the dev account uses `cdk:deploy:shared`.
