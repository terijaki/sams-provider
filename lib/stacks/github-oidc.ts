 ...
 * BudgetStack, or via prod CI after merge to `main`. Never via default
 * `cdk deploy --all` or feature-branch deploys. The role must already exist
 * before Actions can assume it, so the first deploy is local (SSO) into each
 * account.
 */
