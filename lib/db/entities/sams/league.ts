...
import { samsLeaguePk } from "../../key-constants";
import { SamsTable } from "../../tables/sams-table";
import { samsMetadataKeys } from "../keys/sams-keys";

export const SamsLeagueEntity = new Entity({
  name: "SamsLeague",
  table: SamsTable,
  timestamps: false,
  schema: item({
    uuid: string().key(),
    type: string().const("league"),
    name: string(),
    associationUuid: string(),
    seasonUuid: string(),
    leagueHierarchyUuid: string().optional(),
    ...
    pk: string()
      .key()
      .savedAs("pk")
      .link<typeof prevSchema>(({ uuid }) => samsLeaguePk(uuid))
      .hidden(),
    ...samsMetadataKeys(),
    gsi1pk: string()
